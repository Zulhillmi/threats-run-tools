...
export default async function ToolDetailPage({ params }: { params: Params }) {
  ...
  return (
    <section className="section shell detail">
      <article className="detail-main">
        {tool.imageUrl && <img className="detail-image" src={tool.imageUrl} alt="" />}
        <p className="kicker">{tool.toolType}</p>
        <h1>{tool.name}</h1>
      ...
      <aside className="side">
        <div className="card">
          <span className="pill accent">{tool.pricingModel}</span>
          <h3>Visit {tool.name}</h3>
          <p>Open the official site in a new tab.</p>
          <a className="button" href={`/api/outbound?tool=${encodeURIComponent(tool.slug)}&url=${encodeURIComponent(tool.websiteUrl)}`} rel="nofollow">Open website</a>
          {tool.githubUrl && <a className="button ghost" href={tool.githubUrl}>GitHub</a>}
        </div>
        <div className="notice">Spot outdated info? Submit a correction and we’ll review it.</div>
      </aside>
    </section>
  );
}
